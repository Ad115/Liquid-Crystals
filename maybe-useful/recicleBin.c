...
class Distance {
    ...
    public: 
        Distance(Vector dr, float dr_squared)
            : diff_vector(dr), squared_distance(dr_squared) {}                                                    
}

//Function that calculates the distance between 2 particles
        double distance( Particle other, Box &container ) {
            //Calculate the distance using periodic boundaries.

            Vector dr = container.minimum_image(position, other->position);
            double squared_distance = dr * dr;
            
            return Distance{ dr, squared_distance };
        };

//In this function we can think of a PACMAN game, if you get out
        //of the box from the right, the particle will appear at the left wall
        Vector minimum_image(Vector& p1, Vector& p2) { /*
            * Get the minimum distance vector from p2 to p1.
            */
            auto dr = p1 - p2;
            for (int D=0; D<dimensions; D++) {

                double L = side_length[D];
                if (dr[D] <= -L/2) {
                    dr[D] += L
